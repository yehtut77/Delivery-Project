...
<%  String username = (String)session.getAttribute("username");
		...
		String s1 = request.getParameter("rcvd_code");
				...
				<div class="page-inner">
					<div class="page-header">
						<ul class="breadcrumbs">
							...
							<li class="nav-item"><a
								href="rcvdDetail.jsp?agent_code=<%=s1%>">View Receive Detail</a></li>
							
							<li class="separator"><i class="flaticon-right-arrow"></i></li>
							<li class="nav-item"><a href="rcvdAdd.jsp">Add</a></li>
							
						</ul>
					</div>
										
<% 
		System.out.println("rcvd_code="+request.getParameter("rcvd_code"));	
		System.out.println("company_code="+request.getParameter("company_code"));
		PreparedStatement pre_rcvd=null;
		
			
			pre_rcvd=con.prepareStatement("Select * from rcvdmode where rcvd_code=? and company_code=?");
			pre_rcvd.setString(1, request.getParameter("rcvd_code"));
			pre_rcvd.setString(2, request.getParameter("company_code"));
			
	
	
		ResultSet rs_rcvd = pre_rcvd.executeQuery();
		      ...
		      %>
					...
					<div class="card">
						<div class="card-body">
							<div class="card-title">Edit Receive Type</div>
						</div>
					</div>

<form action="RUU" method="post" enctype='multipart/form-data' id="curr_form" >
...
<div class="card">
	<div class="card-body">
		...
		<div class="col-12 col-lg-6">
				...
				<select required class="browser-default custom-select"  name="company_code"  id="company_code" >
				
	<%
	PreparedStatement pre_cmp=null;
	if(companyType.equalsIgnoreCase("main")) 	pre_cmp=con.prepareStatement("select company_code,company_name from company ");
	else{
		
		pre_cmp=con.prepareStatement("Select company_code,company_name from company where  company_code=?");
	...
	ResultSet rs_cmp=pre_cmp.executeQuery();
		...
		if(rs_cmp.getString("company_code").equalsIgnoreCase(request.getParameter("company_code")))
		{	
	%>
						<option value="<%=rs_cmp.getString("company_code") %>" selected ><%=rs_cmp.getString("company_name") %></option>
		...
		<%}
				...
				<div class=" form-group">
						<label class="pure-material-textfield-outlined"> 
								<input type="text" value="<%=rs_rcvd.getString("rcvd_description")%>" id="rcvd_description" name="rcvd_description" required>
                    			<span><i style="color:red; margin-top:1px; font-size:20px;">*</i>Payment Description</span>
           			 	</label>
                     	<span id="rcvd_description_error"></span>
			   </div>
		...
		<div class="col-12 col-lg-6"> 
			<div class="form-group">											
			 		<label class="pure-material-textfield-outlined"> 
							<input  type="text"  class="form-control" value="<%=rs_rcvd.getString("rcvd_code")%>" name="rcvd_code" maxlength="2"  readonly>
			               	<span><i style="color:red; margin-top:1px; font-size:20px;">*</i>Receive Code</span>
			         </label>
			 </div>
			...
			<div class="form-group">
				<label class="pure-material-textfield-outlined">
					...
					<%
 	         			String rcvd_valid=rs_rcvd.getString("valid_code");
					  %>
					<%if (rcvd_valid.equalsIgnoreCase("t")){ %>
					         <input type="radio" checked name="rcvd_validcode"  value="t">valid &nbsp;&nbsp;
                              <input type="radio" name="rcvd_validcode"  value="f">invalid
					<%} else if(rcvd_valid.equalsIgnoreCase("f")){%>	 
									<input type="radio"  name="rcvd_validcode"  value="t">valid &nbsp;&nbsp;
                                     <input type="radio" name="rcvd_validcode" checked value="f">invalid
...
$(document).ready(function(){
 	...
 	$("#rcvd_description").focusout(function() {
		
			//var urname= $('#focusout').val();
			var rcvd_description=document.getElementById("rcvd_description").value;// payment_companycode
			var company_code=document.getElementById("company_code").value;
			//alert(company_code);
			//city_name.value = city_name;
			 $.ajax({
			    		url: "fetch_rcvdAdd.jsp",
			    		type: 'POST',
			    		data: {company_code: company_code,rcvd_description:rcvd_description},
			    		success: function(data) {
			    		//$('#city_name').val(data);
			    		//alert(data);
			    			var ur_name = data.trim();
			    		//alert(ur_name);
				    		var error = document.getElementById("rcvd_description_error");
				            if (ur_name=="ok")  
				            { 
				            	// Changing content and color of content 
				            	document.getElementById("rcvd_description").value="";
				                error.textContent = "***Receive Code Description's already existed***";                
