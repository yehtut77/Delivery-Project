            ...
            <div class="row">
                <div class="col-12">
                    <div class="breadcrumb-content">
                        <h3>Company Received And Delivered Report</h3>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="index.jsp"><i class="icon_house_alt"></i> Home</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Company Received And Delivered Report</li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
<div class="form-group"> 
 ...
 <br><br>
  ...
  <tbody>
   ...
   <div class="col-4">
    <div class="form-group"> 
	  <br>
    	 <label id="lb1" style="font-size:15px;"><b> Total Received</b></label>   
   ...
   <div class="col-4">
    <div class="form-group"> 
	  <br>
    	 <label style="font-size:15px;"><b>Total Delivered </b></label>   

    </div>  </div>
   <div class="col-4">
     <div class="form-group"> 
	  <br>
      			<input type="text" value="" style="width:80px;" name="" id="tdtd" placeholder=" " class="form-control" disabled/>
 ...
 <div class="col-12" >
	<div class="card" id="ca2">

  <div class="m" id="what" style="margin:10px;">
  <div class="row"><div class="col-12"><h5>Transaction Details	
</h5></div></div>
 <div class="row">
 <div class="col-12">
...
var start2="N";
  ...
  start1="Y";
    ...
    if(d1<d2){
	  ...
	  var date1=$('#fdate').val();
	  var date2=$('#tdate').val();
	    $("#basic-datatables-fir tbody tr").remove(); 
...
var data4='';
	        $.ajax({
			    		...
			    		var data2=data.trim();
			    		 var res = data2.split("|");
			    		 var des=res[4].split("*");
			    		 for(var d=1;d<=res[5];d++){
			    			 var dess=des[d].split("^");
			    			 $.ajax({
						    		url: "CRADRC",
						    		type: 'POST',
						    		data: {
	  ...
	  if(vvv==1){}else{
		  ...
		  var t=apd.split("*");
			...
			var d=t[dd].split("^");
			     ...
			     "</tr>";
			     var mk2=markup.trim();
		$("#basic-datatables-fir tbody").append(mk2);}
			
			
	
}	
			$("body").delegate("#clk", "click", function(){

			  var csst=$('#cst').val();
			  var dsst=$('#dst').val();
			  var resss=dsst.split(",");
			  for(var i=1;i<=csst;i++){
				    var dess = resss[i].split(":");

				   var markup2 ="<tr>"+"<td>"+dess[0]+"</td>"+
				                                "<td>"+dess[1]+"</td>"+
				                                "<td>"+dess[2]+"</td>"+
				                                "<td>"+dess[3]+"</td>"+ 
				                                "<td class='markcen'>"+dess[4]+"</td>"+ 
				                                "<td class='markcen'>"+dess[5]+"</td>"+ 
										     "</tr>";
	  	...
	  	$("body").delegate("#clk2", "click", function(){
new sortByDate();
	  });
	  	$("body").delegate("#clk3", "click", function(){
new ajaxcall();
			
	  });
	  </script>
	  <script>  function openWindowWithPost(url, windowoption, name, para1,para2,para3,para4,para5,para6,para7)
	  {
...
openWindowWithPost("CRADRPDF",'statusbar=no','title',para1,para2,para3,para4,para5,para6,para7);
	  }
...
var table, rows, switching, i, x, y, shouldSwitch;
  table = document.getElementById("basic-datatables-fir");
  switching = true;
  ...
  var tbody = document.querySelector("#basic-datatables-fir tbody");
 ...
 var myTab = document.getElementById('basic-datatables-sec');

         for (ii = 1; ii < myTab.rows.length; ii++) {

          
            var objCells2 = myTab.rows.item(ii).cells;

              var aaaa=objCells2.item(0).innerHTML;
              var bbbb=objCells2.item(1).innerHTML;
              var cccc=objCells2.item(2).innerHTML;
              var dddd=objCells2.item(3).innerHTML;
              for (iii = 1; iii < myTab.rows.length; iii++) {

          if(ii==iii){}else{
            var objCells3 = myTab.rows.item(iii).cells;

              var aaaaa=objCells3.item(0).innerHTML;
              var bbbbb=objCells3.item(1).innerHTML;
              var ccccc=objCells3.item(2).innerHTML;
              var ddddd=objCells3.item(3).innerHTML;
              if(aaaa==aaaaa && bbbb==bbbbb && cccc==ccccc && dddd==ddddd){
                myTab.rows[iii].remove();
               myTab.rows[ii].cells[4].innerHTML="&#10003;";
               myTab.rows[ii].cells[5].innerHTML="&#10003;";
}
           }
        }
        }    
 ...
 <input type="text" id="apd" hidden > <input type="text" id="cst" hidden ><input type="text" id="fkts" hidden >
  <input type="text" id="dst" hidden>
 <input type="button" id="clk" value="PLZ" hidden><input type="button" id="clk2" value="PLZ2" hidden><input type="button" id="clk3" value="PLZ2" hidden>
